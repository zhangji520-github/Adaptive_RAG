...
from pymilvus import DataType, Function, FunctionType, IndexType, MilvusClient
from pymilvus.client.types import MetricType

from env_utils import COLLECTION_NAME, MILVUS_URI
from llm_utils import openai_embedding, qwen_embeddings
...
class MilvusVectorSave:
    ...
    def create_collection(self,collection_name: str = COLLECTION_NAME, uri: str = MILVUS_URI, is_first: bool = False):
        ...
        # 稠密向量索引 当然你也可以在 建立连接到时候传入 index_Params 重要的参数就是 index_type 和 metric_type
        index_params.add_index(
            field_name="dense",
    ...
    def add_documents(self, documents: list[Document]):
        """
        添加新的 document 数据到 连接好的数据库 vector_stored_saved
